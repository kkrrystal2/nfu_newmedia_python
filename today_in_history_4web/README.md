...
## 输入：
用户输入日期（月、日）：上行输入月份，下行输入日期。在交互界面使用HTML5之[name='user_date'标签]及[type='date'标签]选日期元件(http://www.runoob.com/tags/tag-body.html)
## 输出：
...
4. 前端浏览器收到web 响应：出现HTML页面有HTML表单的输入 input 类型(type) 为"date"，变数名称(name)为'date'，[templates/entry.html](templates/entry.html)    
